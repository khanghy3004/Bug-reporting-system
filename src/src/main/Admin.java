...
public class Admin extends Customer {
    ...
    // Method show biil and report by Username of account
    public void customerOnBillandReport() throws ParseException {
        if(this.connect.checkUsernameInBill(this.cn)) {
    ...
    // Method show all reports and handle them
                    ...
                    switch(option) {
                        case "1": 
                            for (int i = 0; i < this.listReport.size(); ++i) {
                                ...
                                if (data.getStatus() == 0) {
                                    System.out.println(data.getReportID()+"\t"+new SimpleDateFormat("dd-MM-yyyy hh:mm:ss").format(new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").parse(data.getDateReport())));
                                }
                            }
    ...
    public void insertNewBill() {
        ...
        do {
            ...
            user = input.nextLine();
            // Check user has exist on Account
            if(this.connect.checkUsernameExist(this.cn, user)) break;
        } while(true);
